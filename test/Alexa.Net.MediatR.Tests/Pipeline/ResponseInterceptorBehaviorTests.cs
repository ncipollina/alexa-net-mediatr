...
public class ResponseInterceptorBehaviorTests
{
    public class ResponseInterceptor : IResponseInterceptor
    {
        ...
        public Task Process(IHandlerInput input, SkillResponse output, CancellationToken cancellationToken = default)
        {
            var outputSpeech = output.Response.OutputSpeech as PlainTextOutputSpeech;
            outputSpeech!.Text += "From Interceptor";
    ...
    public async Task Send_WithInterceptor_ShouldChangeLocal()
    {
        ...
        outputSpeech!.Text.Should().Be("From HandlerFrom Interceptor");
    }
    ...
    private static ISkillMediator CreateDefaultSkillMediator(SkillRequest request)
    {
        ...
        serviceCollection.AddTransient<IRequestHandler<IntentRequest>, ResponseInterceptor.RequestHandler>();
        serviceCollection.AddTransient<IResponseInterceptor, ResponseInterceptor>();
        serviceCollection.AddTransient(typeof(IPipelineBehavior), typeof(ResponseInterceptorBehavior));
